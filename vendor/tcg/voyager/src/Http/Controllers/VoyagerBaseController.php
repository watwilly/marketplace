...
class VoyagerBaseController extends Controller
{
    ...
    public function show(Request $request, $id)
    {
        ...
        $adm_tienda = NULL;
        if ($slug == 'tiendas') {
            $publicaciones = \App\Models\Post::where('user_id', $dataTypeContent->user_id)->paginate(50);
            $adm_tienda = $dataTypeContent->userId()->first();
        }

        return Voyager::view($view, compact('dataType', 'publicaciones', 'adm_tienda', 'dataTypeContent', 'isModelTranslatable', 'isSoftDeleted'));
    }
    ...
    public function order(Request $request)
    {
        ...
        if ($slug == "categories") {
            $results = $model->orderBy($dataType->order_column, $dataType->order_direction)->where('home', 1)->whereNull('parent_id')->get();
        }else{
