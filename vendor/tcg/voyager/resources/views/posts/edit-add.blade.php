                        ...
                        <div class="panel-body">
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" name="category_id" id="category_id">
                                    ...
                                    @foreach(Voyager::model('Category')::whereNull('parent_id')->where('name', '<>', 'Servicios')->orderBy('name', 'ASC')->get() as $category)
                                ...
                                @if($imagenes->count() > 0)
                                    @foreach($imagenes as $img)
                                        <div class="col-md-6" style="margin-bottom: 5px;">
                                            <a href="/admin/posts/{{$dataTypeContent->id}}/img/{{$img->id}}/imgdelete">Eliminar</a>
                                            <img src="{{Voyager::get_image($img->imagen, 'small', $img->storage)}}" class="img-responsive" >
                                        </div>
                                    @endforeach
                                @endif
                            @endif
    ...
    <script>
        ...
        $(document).ready(function(){
            $("#category_id").change(function () {
                ...
                elegido=$(this).val();
                $.post("/admin/lista_subcategoria", { _token: "{{ csrf_token() }}" , elegido: elegido }, function(data){
            ...
            @if(isset($dataTypeContent->category_id))
                $.post("/admin/lista_subcategoria", { _token: "{{ csrf_token() }}" , subcategoryId:{{$dataTypeContent->subcategoryId}}, elegido: {{$dataTypeContent->category_id}} }, function(data){
                    $("#subcategoryId").html(data);
                });             
            @endif
        ...
        $(document).ready(function(){
            $("#subcategoryId").change(function () {
                ...
                elegido=$(this).val();
                $.post("/admin/lista_marca", { _token: "{{ csrf_token() }}" , elegido: elegido }, function(data){
                    $("#marca_id").html(data);
                });     
                });
            })

            @if(isset($dataTypeContent->subcategoryId) && $dataTypeContent->marca_id)
                $.post("/admin/lista_marca", { _token: "{{ csrf_token() }}" ,marca_id:{{$dataTypeContent->marca_id}}, elegido: {{$dataTypeContent->subcategoryId}} }, function(data){
                    $("#marca_id").html(data);
                });
            @endif
        ...
        $(document).ready(function(){
            $("#marca_id").change(function () {
                ...
                elegido=$(this).val();
                $.post("/admin/lista_modelos", { _token: "{{ csrf_token() }}" , elegido: elegido }, function(data){
                    $("#modelo_id").html(data);
                });     
                });
            })
            @if(isset($dataTypeContent->marca_id))
                $.post("/admin/lista_modelos", { _token: "{{ csrf_token() }}" , modelo_id:{{$dataTypeContent->Modelos_id}}, elegido: {{$dataTypeContent->marca_id}} }, function(data){
                    $("#modelo_id").html(data);
                }); 
            @endif
        });
