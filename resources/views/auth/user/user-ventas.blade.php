                        ...
                        <div class="grid p-3 mb-3 rows-4 cols-2 rows-md-2 cols-md-4 rows-lg-1 cols-lg-6 align-items-center card card-hover">
                            <div class="grid-item c-2 c-md-4 c-lg-2">
                                <a href="posts.show.php" class="custom-link link-secondary">
                                  <h5 class="m-0 text-truncate">{{$ventas->titulo}}</h5>
                                </a>
                            </div>
                            <div class="grid-item">
                                <h6 class="m-0 text-muted">{{$ventas->buyer_name}}</h6>
                            </div>
                            <div class="grid-item justify-self-end justify-self-lg-end">
                                <h6 class="m-0 status-{{$ventas->status}} ">{{$ventas->status}}</h6>
                            </div>
                            <div class="grid-item c-2 c-md-1 justify-self-center justify-self-lg-end">
                                <h6 class="m-0">ARS ${{$ventas->total_paid_amount}}</h6>
                            </div>
                            <div class="grid-item c-2 c-md-1 justify-self-center justify-self-md-end">
                                <a href="/detail-sales/{{$ventas->id}}/{{$ventas->ref_id}}" class="section__link section__link--success">Detalles</a>
                            </div>
                        </div>
                      @endforeach
                        <div class="col-12">
                            <div class="d-flex justify-content-center">
                              {{$query->render()}}
                                <!--<a href="#" class="btn btn-primary">Cargando más ventas...</a>-->
                            </div>
                        </div>
                    </div>
                @else
                    <div class="alert alert-info">No tienes ventas realizadas</div>
                @endif
            </div>
        </div>
    </div>
